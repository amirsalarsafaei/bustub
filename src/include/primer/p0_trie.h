...
class TrieNode {
 public:
   ...
   */
  bool HasChild(char key_char) const { return HasChildren() && children_.find(key_char) != children_.cend(); }
...
class Trie {
 private:
  ...
  std::unique_ptr<TrieNode> *FindPtr(const std::string &key) {
    auto node_ptr = &root_;
    u_long key_char_idx = 0;
    while (node_ptr != nullptr && key_char_idx < key.size()) {
      char ch = key[key_char_idx];
      auto node = node_ptr->get();

      if (node->HasChild(ch)) {
        node_ptr = node->GetChildNode(ch);
      } else {
        node_ptr = nullptr;
      }
      key_char_idx++;
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    while (node_ptr != nullptr && key_char_idx < key.size()) {
      ...
      if (node->HasChild(ch)) {
        node_ptr = node->GetChildNode(key[key_char_idx]);
      } else {
        node_ptr = node->InsertChildNode(ch, std::make_unique<TrieNode>(ch));
      }
      key_char_idx++;
    }
    if (node_ptr == nullptr || key_char_idx != key.size()) {
      latch_.WUnlock();
      return false;
    }

    auto last_node_ptr = node_ptr;
    auto last_node = last_node_ptr->get();

    if (last_node->IsEndNode()) {
      latch_.WUnlock();
      return false;
    }

    last_node_ptr->reset(new TrieNodeWithValue<T>(std::move(**last_node_ptr), value));
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    node_ptr->get()->SetEndNode(false);
    while (path.size() > 1 && !path.back()->get()->IsEndNode() && !path.back()->get()->HasChildren()) {
      auto del_node = path.back()->get();
      path.pop_back();
      auto del_node_parent = path.back()->get();
      del_node_parent->RemoveChildNode(del_node->GetKeyChar());
    }
    latch_.WUnlock();
    return true;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto node_ptr = FindPtr(key);
    *success = node_ptr != nullptr && node_ptr->get()->IsEndNode();

    if (*success) {
      auto value = static_cast<TrieNodeWithValue<T> *>(node_ptr->get())->GetValue();
      latch_.RUnlock();
      return value;
    }
    latch_.RUnlock();
